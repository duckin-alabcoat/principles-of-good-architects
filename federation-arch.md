...
The federation is the **only** path by which a learning from one system becomes a behavioral rule in another's Architect.
...
| Principles registry | `principles/master.md` | Canon-only per [ADR-0010](adr/0010-registries-canon-only-sidecar-history.md); entry status flips track lifecycle. Gitignored as data per [ADR-0007](adr/0007-github-as-system-storage-data-excluded.md). |
| Principles change history | `principles/master-history.md` | Append-only sidecar per [ADR-0010](adr/0010-registries-canon-only-sidecar-history.md). Gitignored. |
| Habits registry | `habits/master.md` | Canon-only per [ADR-0010](adr/0010-registries-canon-only-sidecar-history.md). Gitignored. |
| Habits change history | `habits/master-history.md` | Append-only sidecar per [ADR-0010](adr/0010-registries-canon-only-sidecar-history.md). Gitignored. |
...
Initial version `0.1.0` reflects bootstrap status. `1.0.0` lands when:
- At least one producer file exists in another Architect's repo and has been ingested,
- At least one principle or universal habit has been shipped as a role-doc edit and accepted in another Architect's repo, and
- This role doc has been reviewed by an Auditor engagement.
...
- **Incidental data in committed system files.** Session-handoff.md carries direct user quotes, machine names, and home-directory paths; federation-arch.md `Data root:` field pins a user-specific filesystem path; portfolio.md names the user's personal-system inventory. Repo is private, but per [ADR-0007](adr/0007-github-as-system-storage-data-excluded.md)'s classification rule this is data-shaped content inside system files. Flagged 2026-05-26 (session 11) for deliberate handling later — current stance is acceptable while repo remains private. Revisit before any access widening or public-mirror consideration.
