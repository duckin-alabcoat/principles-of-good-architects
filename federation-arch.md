...
- **1.1.2** (2026-05-28) — Onboarding reframed agent-driven (session 15), kit bumped v0.6.1 → **v0.6.2**. [PROCESS.md](PROCESS.md) §"Fresh-Architect onboarding steps" now leads with a "How to run this — agent-driven by default" note: the runbook is executed by Claude (running as the Federation Architect per [ADR-0017](adr/0017-bootstrapping-is-a-federation-responsibility.md)), which gathers token values from the operator via a short intake and does the substitution — rather than the operator hand-doing find-and-replace. Step 5 retitled "Fill in the template tokens" and rewritten to instruct the agent to *ask* only about genuine choices (project naming, data-root path, how to address the user) while auto-detecting what the system already knows (timezone, machine) rather than asking for it. [bootstrap-kit/README.md](bootstrap-kit/README.md) "drives the kit" line corrected to match. Driven by the observation that manual multi-file token find-and-replace is error-prone and avoidable — the system is built to be agent-driven. Patch — process/kit doc change with no federation-arch behavioral surface change; precedent at v1.1.1.
- **1.1.1** (2026-05-28) — Kit/process doc-completeness fix (session 15), kit bumped v0.6.0 → **v0.6.1**. Triggered by a "could the public version bootstrap an Architect on Linux?" review: the kit is OS-portable (machine-detect is `hostname`-primary with `scutil` only an optional macOS nicety; timestamps via `TZ=… date`; `.claude/settings.json` is git/gh/mkdir/ls/cat with no macOS-locked command), but [PROCESS.md §5](PROCESS.md)'s token table was missing ~12 tokens the kit templates actually use — most consequentially `<<USER_NAME>>` (used throughout the role-doc and CLAUDE.md templates with no inline guidance), plus `<<REPO_OWNER>>`, `<<REPO_NAME>>`, `<<HABITS_TABLE>>`, `<<SYSTEM_SPECIFIC_PRINCIPLES>>`, `<<VOICE_STYLE_BULLETS>>`, the `<<EXTRA_*>>` and `<<ARCHITECT_SPECIFIC_*>>` rows. A literal find-and-replace from the table would have left unsubstituted placeholders (not Linux-specific — any-OS bootstrap). PROCESS.md §5 table completed; stale counts refreshed across PROCESS.md, [bootstrap-kit/README.md](bootstrap-kit/README.md), and [role-doc-template.md](bootstrap-kit/role-doc-template.md) (10 principles / 15 habits → 13 / 21; template CHANGELOG and §4 set-stamps corrected to the v1.1.0 set). Patch — kit/process structural change with no federation-arch behavioral surface change; precedent at v0.11.1.
